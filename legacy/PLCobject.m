classdef PLCobject
   ...
   methods
       ...
       function obj = CalculateGradient(obj, kernel_value, cutlinelength, varargin)
       ...
       % ---> this example masks the grains with a phase number of 2
            ...
            obj.max_gradient = max(obj.gradient(:));
...
%                     values_length = sum(isnan(gradline(:,i))==0);
%                     gradlinesum(i) = nansum(gradline(:,i))/values_length;
%                     gradlinesum(i) = peak2peak(gradline(:,i));
%                     nullcheck_row = ismember(gradlinerow,obj.rownull);
%                     nullcheck_col = ismember(gradlinecol,obj.colnull);
...
%                     end
       ...
       function [gradline, gradlinerow, gradlinecol, gradlinesum, graddist, i_coords] = GradientLineTranslation(obj)
            pivots = 5; % angle of rotation in radians (pi/144 is default)
            graddist = zeros(pivots,1);
            gradline = zeros(obj.cutlinelength+1,pivots);
            gradlinerow = zeros(obj.cutlinelength+1,pivots);
            gradlinecol = zeros(obj.cutlinelength+1,pivots);
            gradlinesum = zeros(1,pivots);
            i_coords = zeros(pivots,4);
            ix1 = zeros(pivots,1);
            iy1 = zeros(pivots,1);
            ix2 = zeros(pivots,1);
            iy2 = zeros(pivots,1);
...
%                     end
       ...
       % interest.      
            [maxgradrow,maxgradcol] = find(obj.gradient == obj.max_gradient);
            ...
            if isempty(obj.gradient_cutline) == 0
                gradient_vector = obj.gradient(:);
                gradient_vector = sort(gradient_vector(:),'descend','MissingPlacement','last'); % for r2017a and above
%                 gradient_vector = sort(gradient_vector(:),'descend');
...
%         obj.CutLineMaxGradient; % recalculate the maxmimum
            % gradient after masking grains
%         obj.SetCutLine;
...
%            for i = 1:size(obj.gradient_cutline,1)
%                if ismember(obj.gradline_row_round,obj.rownull) == 1
%                loc_gradline = [obj.gradline_row_round(i),obj.gradline_col_round(i)];
%                loc_nullspace = [obj.rownull);
%                end
%            end
%            [obj.nullcheck_row,obj.locb_row] = ismember(obj.gradline_row_round,obj.rownull);
%            [obj.nullcheck_col,obj.locb_col] = ismember(obj.gradline_col_round,obj.colnull);
%            obj.nullcheck = obj.locb_row & obj.locb_col;
%            obj.nullcheck = obj.nullcheck_row & obj.nullcheck_col;
        
%        [obj.nullcheck,obj.locb_coords] = ismember(obj.gradline_coords_round,obj.null_coords,'rows');
%         obj.nullcheck = ismember(obj.gradline_coords_round,obj.locb_coords);
%         obj.gradient_cutline(obj.nullcheck==1) = NaN; % if the gradient 
         
               % line crosses a nulled grain, null that portion of the gradient line
%            if any(isnan(obj.gradient_cutline) == 1)
%                obj.gradient_cutline(obj.nullcheck==1) = NaN;
%                %gradline(:,i) = NaN;
%            end
       end
